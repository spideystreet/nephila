...
Le LLM est configuré via **OpenRouter** (modèle configurable). Il est lié aux 4 outils disponibles via `llm.bind_tools(TOOLS)`.
...
L'état est défini dans `agent/model_state.py` (Pydantic + `add_messages`) :
...
| `interactions_checked` | `bool` | Mis à `True` par le nœud guardrail |
